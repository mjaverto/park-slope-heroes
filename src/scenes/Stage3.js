...
// Display labels for each phase of the colossus.
const BOSS_LABEL_P1 = 'THE SEWER LORD';
const BOSS_LABEL_P2 = 'THE SEWER COLOSSUS';
...
export class Stage3 extends Phaser.Scene {
  ...
  init(data) {
    // Stage 2 hands us { score, lives } on transition. Standalone dev starts
  ...
  preload() {
    ...
    // Track missing Stage 3 assets (bg + boss + music) so create() can fall back.
    ...
    // Stage 3 background tiles — Prospect Park / Grand Army Plaza arch.
    this.load.image('bg-stage3-tile1', './assets/backgrounds/stage3-tile1.png');
    this.load.image('bg-stage3-tile2', './assets/backgrounds/stage3-tile2.png');
    this.load.image('bg-stage3-tile3', './assets/backgrounds/stage3-tile3.png');

    // Sewer Colossus — two phases, 6 sprites each. Missing textures flip the
    // SewerColossus entity to a tinted-rect fallback per phase.
    for (const phase of ['p1', 'p2']) {
      for (const suffix of ['idle', 'walk-1', 'walk-2', 'attack', 'hit', 'defeat']) {
        const key = `sewer-colossus-${phase}-${suffix}`;
  ...
  create() {
    ...
    // Background tiles — Stage 3 palette: deep park green → stone plaza →
    // sewer black-green. Fallback rectangles if tiles are missing.
    const fallbackColors = [0x1f3a24, 0x5c5a4e, 0x0c1a10];
    for (let i = 0; i < 3; i++) {
      const key = `bg-stage3-tile${i + 1}`;
    ...
    // Player — spawn at the park entrance.
    this.player = new Player(this, PLAYER_SPAWN_X, PLAYER_SPAWN_Y, this.chosenKey);

    // Stage data (STAGES[2]). `sections` drives scroll-lock triggers.
    this.stage = getStage(2);
    ...
    this.add.text(512, 24, 'Stage 3 — Grand Army Plaza', {
      fontFamily: 'monospace',
      fontSize: '22px',
      color: '#ffffff',
    ...
    // Stage 3 music (loaderror-tolerant — startMusic no-ops if the key is missing).
    this.sound_mgr.startMusic('music_stage3');
  }
  ...
  _createBossHud() {
    this.bossHud = {};
    this.bossHud.label = this.add.text(512, BOSS_BAR_Y - 18, BOSS_LABEL_P1, {
      fontFamily: 'monospace',
      fontSize: '16px',
      color: '#ffffff',
      stroke: '#000000',
  ...
  // Callback the SewerColossus calls on phase transition — swaps the HUD
  // label to the more dramatic "COLOSSUS" name when phase 2 kicks in.
  onBossPhaseChange(newPhase) {
    if (!this.bossHud) return;
    if (newPhase === 2) {
      this.bossHud.label.setText(BOSS_LABEL_P2);
      this.bossHud.label.setColor('#ff5252');
      // Flash the label for drama.
      this.tweens.add({
        targets: this.bossHud.label,
        scale: { from: 1.3, to: 1 },
        duration: 400,
        ease: 'Back.easeOut',
      });
  ...
  _spawnBoss(section) {
    ...
    this.boss = new SewerColossus(this, spawnX, spawnY, {
      key: 'sewer-colossus',
      hp: 250,
      phase1: { damage: 15, speed: 80, spriteKey: 'sewer-colossus-p1' },
      phase2: { damage: 22, speed: 120, spriteKey: 'sewer-colossus-p2' },
      contactDamage: 12,
    });
    this.boss.onDefeat = () => this._onBossDefeated();
    // Reset label in case of restart.
    if (this.bossHud) {
      this.bossHud.label.setText(BOSS_LABEL_P1);
      this.bossHud.label.setColor('#ffffff');
    }
